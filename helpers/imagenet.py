...
class PatchedImageNet(ImageFolder):
    ...
    def __init__(self, root: Union[str, Path], meta_file=None, split: str = "train", **kwargs: Any) -> None:
        root = self.root = os.path.expanduser(root)
        self.meta_dir = meta_file
        self.split = verify_str_arg(split, "split", ("train", "val"))

        self.parse_archives()
        wnid_to_classes = load_meta_file(self.meta_dir)[0]
    ...
    def parse_archives(self) -> None:
        if not check_integrity(os.path.join(self.root, META_FILE)):
            parse_devkit_archive(self.meta_dir)

        if not os.path.isdir(self.split_folder):
            if self.split == "train":
                parse_train_archive(self.meta_dir)
            elif self.split == "val":
                parse_val_archive(self.meta_dir)
